...
import type { UsIndustryGdpReport } from "@/lib/gdp-data";
...
export function UsIndustryGdpClient() {
  const searchParams = useSearchParams();
  const statePage = Number(searchParams.get("statePage")) || 1;

  const [report, setReport] = useState<UsIndustryGdpReport | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/gdp")
      .then(async (res) => {
        ...
        if (!res.ok) throw new Error(body.error ?? "Failed to load GDP report");
        return body as UsIndustryGdpReport;
      })
      .then((data) => {
        if (!cancelled) setReport(data);
      })
      .catch((err) => {
        if (!cancelled) setError(err instanceof Error ? err.message : "Failed to load GDP report");
      });
  ...
  if (error) {
    return (
      <div className="rounded-lg border border-dashed px-4 py-3 text-sm text-muted">
        Couldn&apos;t load the GDP dashboard: {error}
      </div>
    );
  }

  if (!report) {
    return <UsIndustryGdpSkeleton />;
  }
