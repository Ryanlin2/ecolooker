...
import type { CfpbReport } from "@/lib/cfpb-data";
...
export function CfpbComplaintsClient() {
  const searchParams = useSearchParams();
  const geoPage = Number(searchParams.get("page")) || 1;
  const anomalyPage = Number(searchParams.get("anomalyPage")) || 1;

  const [report, setReport] = useState<CfpbReport | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/cfpb")
      .then(async (res) => {
        ...
        if (!res.ok) throw new Error(body.error ?? "Failed to load CFPB report");
        return body as CfpbReport;
      })
      .then((data) => {
        if (!cancelled) setReport(data);
      })
      .catch((err) => {
        if (!cancelled) setError(err instanceof Error ? err.message : "Failed to load CFPB report");
      });
  ...
  if (error) {
    return (
      <div className="rounded-lg border border-dashed px-4 py-3 text-sm text-muted">
        Couldn&apos;t load the CFPB dashboard: {error}
      </div>
    );
  }

  if (!report) {
    return <CfpbComplaintsSkeleton />;
  }
