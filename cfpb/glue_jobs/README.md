...
| Function | What it does |
|---|---|
| `read_raw()` | Reads the landed CSV with an explicit string-only schema (`RAW_SCHEMA`) — inferring the schema would cost a second full pass and guess wrong on values like ZIP `"331XX"`. `multiLine=True` is required because narratives contain embedded, RFC4180-quoted newlines, which forces a single-threaded initial read; `repartition(400)` fixes that back up afterward. **`COLUMN_MAP` must match the live file's column set and order exactly** — `header=true` only skips the header line here, it does not map by name, so a schema field that doesn't correspond to a real column silently shifts every later field into the wrong column instead of failing loudly. CFPB's documented schema also lists `"Consumer consent provided?"` and `"Consumer disputed?"`, but neither appears in the current bulk CSV export, so both are intentionally absent from `COLUMN_MAP`. |
| `transform(df)` | The main per-row pipeline: runs the validation/cleaning helpers above, trims the remaining string columns, adds `has_narrative`, computes `record_hash` (NULL-safe SHA-256 over all business columns except ingestion metadata) plus `ingest_ts`/`source_file`, drops rows with a null `complaint_id` or `date_received`, and collapses duplicate `complaint_id`s (keeping the row with the latest `date_sent_to_company`) since Iceberg `MERGE` aborts if a target row matches more than one source row. |
